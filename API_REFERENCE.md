...
### 10.4a Подзадачи (subtasks) — семантика (проверено на живом API, 30.08.2026)

Иерархия задач: у родителя поле `subtasks` — **массив ID дочерних задач**. Связь хранится только на родителе.

| Факт | Значение |
|---|---|
| Подзадача — полноценная задача | свой ID, колонка, стикеры, дедлайн; остаётся видимой в `GET /tasks?columnId=X` |
| Кросс-колоночные / кросс-досочные связи | работают (родитель и ребёнок могут жить на разных досках) |
| Обратной ссылки `parentId` у ребёнка | **нет** — получить родителя через API нельзя |
| `PUT {"subtasks": [...]}` | **полная замена** списка, не append; для добавления — read-modify-write |
| `POST /tasks` с `subtasks` | работает — атомарное создание родителя с детьми |
...
0. **Базы знаний — API отсутствует** (проверено 2026-08-31, дополнено 09-05): в официальной OpenAPI-спеке (GET `/api-json` с Bearer-ключом, 40 путей, 106 схем) нет ни одного эндпоинта/схемы для баз знаний и страниц; живые пробы `/knowledge-bases`, `/knowledge`, `/books`, `/wiki`, `/kb`, `/book-pages`, `/pages`, `/notes`, `/boards/{id}/pages` — 404 `Cannot GET`. **Проверено на реальной базе знаний**: база видна в `GET /boards` как обычная `BoardDto` (id/title/projectId/deleted, поля типа нет), колонок нет (`GET /columns?boardId=` → пусто). **Страницы — не задачи**: полный обход `GET /task-list` (2095 задач, includeDeleted) не находит страницу (по title и по human-ID `OBSCH-592`); человекочитаемый ID страницы выдаётся общим счётчиком проекта (у задач видно `idTaskProject`/`idTaskCommon`), но сущность живёт в отдельном хранилище, доступном только websocket-транспорту веб-интерфейса. Webhooks (location/title/chat_message) про страницы не знают.
