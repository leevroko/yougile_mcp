...
0. **Базы знаний — API отсутствует** (проверено 2026-08-31): в официальной OpenAPI-спеке (GET `/api-json` с Bearer-ключом, 40 путей, 106 схем) нет ни одного эндпоинта/схемы для баз знаний и страниц; живые пробы `/knowledge-bases`, `/knowledge`, `/books`, `/wiki`, `/kb`, `/book-pages`, `/pages`, `/notes`, `/boards/{id}/pages` — 404 `Cannot GET`. **Проверено на реальной базе знаний**: база видна в `GET /boards` как обычная `BoardDto` (id/title/projectId/deleted, поле типа отсутствует), колонок нет (`GET /columns?boardId=` → пусто); страницы и их содержимое недоступны никак. Фича доступна только в веб-версии UI (help.yougile.com).
