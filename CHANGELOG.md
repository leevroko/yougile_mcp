...
### Fixed
- `get_task` показывал `deadline: {}` при реально записанном значении (issue #8): у `valueobject.Deadline` не было `MarshalJSON` — приватное поле сериализовалось пустым объектом. Теперь отдаёт `{"deadline": <ms>}` (формат API); без дедлайна — `null`. Запись и раньше работала корректно (`{"deadline": {"deadline": ms}}`).
